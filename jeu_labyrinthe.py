...
def joue(labyrinthe):
    ...
    mon_plateau = plateau.init(labyrinthe)
    personnage = (0, 0)
    ...
    quitte = False
    if plateau.fabrique_chemin(mon_plateau,personnage,sortie) == []:
        print("il est impossible de trouver la sortie sur ce labyrinthe !!")
        return 
    while not quitte:
        print(plateau.fabrique_chemin(mon_plateau,personnage,fantome))
...
def expert(labyrinthe):
    ...
    if plateau.fabrique_chemin(mon_plateau,personnage,sortie) == []:
        print("il est impossible de trouver la sortie sur ce labyrinthe !!")
        return 
    liste_chemin_court=plateau.fabrique_chemin(mon_plateau,personnage,sortie)
    matrice.set_val(mon_plateau,liste_chemin_court[1][0],liste_chemin_court[1][1],3)
    fantome2 = liste_chemin_court[1]
    affichage_graphique=matrice_graphique.MatriceGraphique(mon_plateau)
    affiche_jeu(mon_plateau, affichage_graphique)
    quitte = False
    while not quitte:
        ...
        personnage = plateau.deplace_personnage(mon_plateau, personnage, direction)
        fantome = plateau.deplace_fantome(mon_plateau, fantome,personnage)
        fantome2= plateau.deplace_fantome(mon_plateau,plateau.deplace_fantome(mon_plateau, fantome2, personnage),personnage)
        affiche_jeu(mon_plateau, affichage_graphique)
        if personnage == fantome:
            fin_du_jeu(False, affichage_graphique)
            quitte = True
